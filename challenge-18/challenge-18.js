...
let dirtyCpfs = ["049-214 3421-1", "210.458.522-05", "735 500 794 - 22", "101.123-131x32"]
let newCpfArr = dirtyCpfs.map(function (value){
...
function formatingCpf(cpf){
    let regex = /(\d{3})(\d{3})(\d{3})(\d{2})/;
    return cpf.replace(regex, function(regex, cap1, cap2, cap3, cap4){
        return cap1 + '.' + cap2 + '.' + cap3 + '-' + cap4;
    });
}

newCpfArr.forEach(function(value){
    console.log(formatingCpf(value));
});
...
console.log("Os meses de janeiro, junho e julho começam com a letra j.".match(/j\w{4}\b/g));
